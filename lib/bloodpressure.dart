...
class _BloodPressurePageState extends State<BloodPressurePage> with SingleTickerProviderStateMixin {
    ...
    try {
      ...
      if (systolic == 0 || diastolic == 0) {
        ...
        prompt = '''
          ...
          and briefly explaining why tracking blood pressure is helpful.
        ''';
      } else {
        // --- THE CLINICAL PROMPT (HAS DATA) ---
        prompt = '''
          The user, $userName, has a current blood pressure of $systolic/$diastolic mmHg.
  ...
  Widget _buildCurrentBpAndAddData() {
    return Row(
  ...
  Widget zoneCard(String title, String value, {bool isWide = false}) {
    return Container(
      height: 95,
      width: isWide ? double.infinity : null, // Stretches sideways on desktop
