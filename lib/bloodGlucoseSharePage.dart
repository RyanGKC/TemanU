...
enum BgHighlight { current, average, zone }

class BloodGlucoseSharePage extends StatefulWidget {
  final double currentBg;
  final double averageBg;
  final String zone;
  final String rangeName;
  final String dateRangeLabel;
  final String userName;

  const BloodGlucoseSharePage({
    super.key,
    required this.currentBg,
    required this.averageBg,
    required this.zone,
  ...
  @override
  State<BloodGlucoseSharePage> createState() => _BloodGlucoseSharePageState();
}

class _BloodGlucoseSharePageState extends State<BloodGlucoseSharePage> {
  final _screenshotController = ScreenshotController();
  BgHighlight _selectedHighlight = BgHighlight.current;
  ...
  void _shareImage() async {
    ...
    try {
      ...
      final imagePath = await File('${directory.path}/BG_Progress.png').writeAsBytes(bytes);

      await SharePlus.instance.share(
        ShareParams(
          files: [XFile(imagePath.path)],
          text: "Here's my blood glucose progress from Temanu!",
        )
      );
    } catch (e) {
        ...
        child: Column(
          ...
          children: [
            ...
            Wrap(
              ...
              runSpacing: 12.0,
              children: [
                _buildFilterChip("Current Reading", BgHighlight.current),
                _buildFilterChip("Daily Average", BgHighlight.average),
                _buildFilterChip("Health Zone", BgHighlight.zone),
              ],
            ),
            const SizedBox(height: 40),
  ...
  Widget _buildFilterChip(String label, BgHighlight highlight) {
  ...
  Widget _buildShareableCard() {
    ...
    switch (_selectedHighlight) {
      case BgHighlight.current:
        title = "Current Level";
        value = "${widget.currentBg.toInt()}";
        subtitle = "mg/dL";
        break;
      case BgHighlight.average:
        title = "Daily Average";
        value = "${widget.averageBg.toInt()}";
        subtitle = "mg/dL";
        break;
      case BgHighlight.zone:
        title = "Health Category";
        value = widget.zone;
        subtitle = "Based on ${widget.currentBg.toInt()} mg/dL";
        break;
    }
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Icon(Icons.water_drop_outlined, color: AppTheme.primaryColor, size: 28),
              Text(
            ...
            children: [
              ...
              Text(
                ...
                style: TextStyle(
                  ...
                  fontSize: _selectedHighlight == BgHighlight.zone && widget.zone.length > 6 ? 48 : 56, 
